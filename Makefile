# This Makefile is for the Time::Duration::Parse::More extension to perl.
#
...
#     ABSTRACT => q[parse natural language time duration expressions]
...
#     DISTNAME => q[Time-Duration-Parse-More]
#     EXE_FILES => []
...
MAKE = make
FULLEXT = Time/Duration/Parse/More
BASEEXT = More
PARENT_NAME = Time::Duration::Parse
DLBASE = $(BASEEXT)
VERSION_FROM = 
OBJECT = 
LDFROM = $(OBJECT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Time/Duration/Parse/More.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Time/Duration/Parse
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Time/Duration/Parse
...
TO_INST_PM = lib/Time/Duration/Parse/More.pm

PM_TO_BLIB = lib/Time/Duration/Parse/More.pm \
...
DISTVNAME = Time-Duration-Parse-More-0.006
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Time/Duration/Parse/More.pm $(INST_MAN3DIR)/Time::Duration::Parse::More.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''parse natural language time duration expressions'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Time-Duration-Parse-More' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires: {}' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: 0.006' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "parse natural language time duration expressions",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Time-Duration-Parse-More",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "0.006"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>parse natural language time duration expressions</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Time/Duration/Parse/More.pm blib/lib/Time/Duration/Parse/More.pm 
